...
public abstract class PromocaoUtil {
	 ...
	 * 
	 * @param lanche
	 *            :será o produto selecionado pelo cliente
	 * @param ingrediente
	 *            : Ingrediente selecionado pelo cliente
	...
	public static Lanche isLight(Lanche lanche, Ingrediente ingrediente) {
		...
		if (lanche == null || ingrediente == null)
			throw new IllegalArgumentException(
					"Ocorreu erro ao validar a promoção IsLight, lanche ou ingrediente está invalido");
		...
		if (lanche.getValor() == null)
			throw new IllegalArgumentException(
					"Ocorreu um erro a realizar o reajuste do valor do lanche na promoção isLight.");
	 ...
	 *            : será o produto selecionado pelo cliente
	 * @param porcao
	 *            : tipo de porção que será calculado
	 * @return será retornado o lanche com o valor atualizado do produto
	 * 
	 *         Ex :
	 * 
	 *         A cada 3 porções de X produto o cliente só paga 2. Se o lanche tiver
	...
	public static Lanche calculoDasPorcoes(Lanche lanche, Ingrediente ingrediente, IngredientesEnum porcao) {
		...
		if (lanche == null || ingrediente == null || porcao == null)
			throw new IllegalArgumentException("Ocorreu algum erro no calculo de valores de porcoes.");
	...
	private static int calcularQuantidadeDePorcao(Lanche lanche, IngredientesEnum porcao) {
		...
		for (Ingrediente calcIngrediente : lanche.getIngredientes()) {
			...
			if (calcIngrediente.getValor() == null)
				throw new IllegalArgumentException(
						"Ocorreu algum erro ao calcular a quantidade de proção, valor do ingrediente inválido.");
